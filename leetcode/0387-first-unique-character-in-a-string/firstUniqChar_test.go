...
type FirstUniqCharCase struct {
	Input string

	Output int
}

func TestFirstUniqCharCase(t *testing.T) {
	var casePath, _ = filepath.Abs("./cases.json")
	payload, err := util.LoadCases[[]FirstUniqCharCase](casePath)
	if err != nil {
		panic(err)
	}
	...
	for i, testCase := range *payload {
		...
		result := firstUniqChar(testCase.Input)
