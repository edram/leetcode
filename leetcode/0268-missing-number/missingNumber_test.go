...
type MissingNumberCase struct {
	Input []int

	Output int
}

func TestMissingNumberCase(t *testing.T) {
	var casePath, _ = filepath.Abs("./cases.json")
	payload, err := util.LoadCases[[]MissingNumberCase](casePath)
	if err != nil {
		panic(err)
	}
	...
	for i, testCase := range *payload {
		...
		result := missingNumber(testCase.Input)
