...
type AccountBalanceAfterPurchaseCase struct {
	Input int
...
func TestAccountBalanceAfterPurchase(t *testing.T) {
	...
	payload, err := util.LoadCases[[]AccountBalanceAfterPurchaseCase](casePath)
	if err != nil {
		panic(err)
	}
	...
	for i, testCase := range *payload {
		...
		result := accountBalanceAfterPurchase(testCase.Input)
