...
type CountSegmentsCase struct {
	Input string

	Output int
}

func TestCountSegmentsCase(t *testing.T) {
	var casePath, _ = filepath.Abs("./cases.json")
	payload, err := util.LoadCases[[]CountSegmentsCase](casePath)
	if err != nil {
		panic(err)
	}
	...
	for i, testCase := range *payload {
		...
		result := countSegments(testCase.Input)
