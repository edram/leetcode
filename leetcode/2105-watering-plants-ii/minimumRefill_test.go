...
func TestMinimumRefill(t *testing.T) {
	...
	payload, err := util.LoadCases[[]MinimumRefillCase](casePath)
	if err != nil {
		panic(err)
	}
	...
	for i, testCase := range *payload {
		...
		result := minimumRefill(testCase.Input.Plants, testCase.Input.CapacityA, testCase.Input.CapacityB)
