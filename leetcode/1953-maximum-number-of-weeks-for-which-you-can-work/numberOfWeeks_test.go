...
type NumberOfWeeksCase struct {
	Input []int

	Output int64
}

func TestNumberOfWeeks(t *testing.T) {
	var casePath, _ = filepath.Abs("./cases.json")
	payload, err := util.LoadCases[[]NumberOfWeeksCase](casePath)
	if err != nil {
		panic(err)
	}
	...
	for i, testCase := range *payload {
		...
		result := numberOfWeeks(testCase.Input)
