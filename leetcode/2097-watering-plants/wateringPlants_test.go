...
type WateringPlantsCase struct {
	Input struct {
		Plants   []int
		Capacity int
	}
...
func TestWateringPlants(t *testing.T) {
	...
	payload, err := util.LoadCases[[]WateringPlantsCase](casePath)
	if err != nil {
		panic(err)
	}
	...
	for i, testCase := range *payload {
		...
		result := wateringPlants(testCase.Input.Plants, testCase.Input.Capacity)
