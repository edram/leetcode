...
type ArrangeCoinsCase struct {
	Input int

	Output int
}

func TestArrangeCoinsCase(t *testing.T) {
	var casePath, _ = filepath.Abs("./cases.json")
	payload, err := util.LoadCases[[]ArrangeCoinsCase](casePath)
	if err != nil {
		panic(err)
	}
	...
	for i, testCase := range *payload {
		...
		result := arrangeCoins(testCase.Input)
