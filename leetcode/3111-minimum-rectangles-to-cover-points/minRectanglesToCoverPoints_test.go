...
type MinRectanglesToCoverPointsCase struct {
	Input struct {
		Points [][]int
		W      int
	}
...
func TestMinRectanglesToCoverPoints(t *testing.T) {
	...
	payload, err := util.LoadCases[[]MinRectanglesToCoverPointsCase](casePath)
	if err != nil {
		panic(err)
	}
	...
	for i, testCase := range *payload {
		...
		result := minRectanglesToCoverPoints(testCase.Input.Points, testCase.Input.W)
