...
type GarbageCollectionCase struct {
	Input struct {
		Garbage []string
		Travel  []int
	}
...
func TestGarbageCollection(t *testing.T) {
	...
	payload, err := util.LoadCases[[]GarbageCollectionCase](casePath)
	if err != nil {
		panic(err)
	}
	...
	for i, testCase := range *payload {
		...
		result := garbageCollection(testCase.Input.Garbage, testCase.Input.Travel)
